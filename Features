...
This application has two tabs, namely **Manage Users** and **Manage Roles**. Depending upon the solutions requirements of Manage Roles we can enable or disable it from configuration but still roles can be managed via SQL directly.

### Manage Users
In the application user can see the list of all users available in database. User can be created, edited and deleted.
Super admin user (username is admin) cannot be deleted, only it's password can be changed. Super admin cannot be disabled too. It is created during setup of User Management for solutions.
...
After user is successfully created, user details can be modified except(user_id). Admin user cannot delete or disable himself.
